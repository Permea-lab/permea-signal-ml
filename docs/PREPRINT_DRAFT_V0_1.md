...
### Leakage-aware split sensitivity

Existing baseline model families were also rerun under the committed `leakage_aware_group_stratified` split manifest at `results/sensitivity/combined_group_stratified_split_manifest.csv`. The manifest assigns 2,959 rows and 2,958 groups across 5 folds while keeping group identifiers together. It is a sensitivity manifest for this specific grouping strategy, not evidence that all leakage, source, or provenance concerns have been resolved.

Under this leakage-aware manifest, the Dummy most-frequent classifier yielded ROC-AUC 0.5000, PR-AUC 0.0909, and MCC 0.0000. Logistic Regression yielded ROC-AUC 0.8587, PR-AUC 0.5024, and MCC 0.3747. Random Forest yielded ROC-AUC 0.8718, PR-AUC 0.5807, and MCC 0.5084. Relative to the random-stratified baseline, Logistic Regression changed by -0.0018 ROC-AUC, +0.0121 PR-AUC, and +0.0130 MCC. Random Forest changed by +0.0229 ROC-AUC, +0.0805 PR-AUC, and +0.0753 MCC.

The sensitivity rerun did not collapse the baseline signal under this specific grouping strategy. Logistic Regression remained broadly similar to its random-stratified baseline, and Random Forest was comparable to or higher than its random-stratified baseline under this manifest. This increases confidence that the initial signal was not solely an artifact of the reconstructed random-stratified split, but it is not leakage-free evidence, robust-generalization evidence, biological validation, or wet-lab validation.
...
Taken together, the regenerated results support a bounded computational interpretation. On the present random-stratified benchmark surface, sequence-derived physicochemical features support non-trivial baseline comparison and computational candidate-prioritization hypotheses beyond a class-prior reference. Because same-label duplicate and high-similarity sequence pairs cross reconstructed folds, the random-stratified performance estimates may be optimistic and should be interpreted as evidence of learnable benchmark signal under the current split rather than stronger generalization evidence. The leakage-aware sensitivity rerun provides a bounded comparison point under one committed group-stratified manifest and did not collapse the baseline signal. These findings remain computational and benchmark-specific. They do not constitute experimental validation, mechanistic proof, leakage-free status, robust generalization, or evidence of broad delivery behavior beyond the current BBB-oriented wedge.
...
Additional benchmark-readiness limitations remain. Original label-source criteria are not yet fully reconstructed. The first leakage audit found no normalized exact-sequence label conflicts, but it did identify same-label exact duplicates, near-duplicate pairs, high-similarity pairs, and cross-fold high-similarity pairs under the reconstructed random-stratified split. These findings create a moderate benchmark optimism risk. The leakage-aware grouping retained a `max_pairs=10000` caveat, produced only one non-singleton group under the current combined grouping output, and could not use source-aware grouping because the source field is too coarse. Adjacent CPP benchmark work reinforces the need for conservative imbalance, deduplication, and validation language in peptide-prediction settings [@practicpp_2024; @perseucpp_2025]. The audit and sensitivity findings do not invalidate the current bounded computational benchmark surface for trusted review, but they require conservative reporting before public preprint use and do not support stronger benchmark claims without additional provenance, grouping, and validation work.
...
Within that framework, the observed baseline results are useful without being overstated. Logistic Regression and Random Forest both exceed the trivial class-prior baseline on the benchmark surface, which supports a cautious argument that non-trivial computational permeability-related benchmark signal is detectable from the feature set used here under the current split. The leakage-aware sensitivity rerun did not collapse that signal under the committed group-stratified manifest, which increases confidence relative to the initial leakage concern. At the same time, the feature surface is narrow, the model family is baseline-oriented, the grouping output remains caveated, and the manuscript does not justify claims of validated biological prediction, leakage-free status, or robust generalization. The appropriate interpretation is therefore practical rather than expansive: the repository supports computational candidate-prioritization hypotheses before experimental validation under a BBB-oriented benchmark frame.
...
The present work contributes an initial computational evidence package for a BBB-oriented peptide benchmark built around sequence-derived physicochemical features and reproducible baseline reruns. Within that bounded scope, the repository now provides a clearer random-stratified benchmark surface for comparing trivial and non-trivial baselines, plus a leakage-aware sensitivity rerun showing that the signal did not collapse under one committed group-stratified manifest. These results support computational candidate-prioritization hypotheses prior to experimental validation, not validated delivery claims.
