...
Computational evidence only. No biological validation, wet-lab validation, therapeutic efficacy, clinical interpretation, leakage-free status, or robust-generalization claim is made.
...
The regenerated Dummy baseline yields a PR-AUC of approximately 0.0909, consistent with class-prior behavior under strong imbalance. Logistic Regression reaches approximately 0.8605 ROC-AUC and 0.4903 PR-AUC, while Random Forest reaches approximately 0.8489 ROC-AUC and 0.5002 PR-AUC. The corresponding MCC values are approximately 0.3618 for Logistic Regression and 0.4331 for Random Forest.

Within the regenerated Random Forest artifact set, the current feature-importance ordering is `gravy`, `aromaticity`, `pI`, `length`, and `charge`. This is a model-level artifact summary, not a mechanistic explanation of BBB transport.
