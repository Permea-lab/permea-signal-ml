...
Computational evidence only. No biological validation, wet-lab validation, therapeutic efficacy, clinical interpretation, leakage-free status, or robust-generalization claim is made. Current primary metrics are random-stratified baseline metrics and may be optimistic under the documented same-label cross-fold duplicate and sequence-similarity findings. Leakage-aware sensitivity reruns are included as bounded sensitivity estimates, not validation or generalization proof.
...
To test whether the baseline signal collapsed under a stricter grouping policy, existing baseline model families were rerun under the committed `leakage_aware_group_stratified` split manifest at `results/sensitivity/combined_group_stratified_split_manifest.csv`. This manifest assigns 2,959 rows and 2,958 groups across 5 folds while keeping group identifiers together; it is a sensitivity manifest, not evidence that all leakage or provenance concerns are resolved.

Under this leakage-aware manifest, the Dummy most-frequent classifier yielded ROC-AUC 0.5000, PR-AUC 0.0909, and MCC 0.0000. Logistic Regression yielded ROC-AUC 0.8587, PR-AUC 0.5024, and MCC 0.3747. Random Forest yielded ROC-AUC 0.8718, PR-AUC 0.5807, and MCC 0.5084. Relative to the random-stratified baseline, Logistic Regression changed by -0.0018 ROC-AUC, +0.0121 PR-AUC, and +0.0130 MCC, while Random Forest changed by +0.0229 ROC-AUC, +0.0805 PR-AUC, and +0.0753 MCC.

These sensitivity results did not collapse the baseline signal under this specific grouping strategy. Logistic Regression remained broadly similar, and Random Forest was comparable to or higher than its random-stratified baseline under this manifest. The result increases confidence that the original signal was not solely an artifact of the initial random-stratified reconstruction, but it is not a leakage-free result, robust-generalization evidence, biological validation, or wet-lab validation.
...
The random-stratified metrics should therefore be interpreted as baseline metrics that may be optimistic under measured sequence-similarity structure. The leakage-aware sensitivity rerun provides a bounded comparison point under one committed group-stratified manifest and did not collapse the baseline signal, but it does not establish leakage-free status or robust generalization. Any stronger benchmark claim would require stronger provenance closure, richer source/family metadata, and additional external or experimental validation. Adjacent CPP benchmark work reinforces the need for conservative imbalance, deduplication, and validation language in peptide-prediction settings [@practicpp_2024; @perseucpp_2025].

## Limitations

Dataset provenance is not fully closed. Attribution and licensing requirements still require confirmation. Original label-source criteria are partially unresolved. The leakage-aware grouping retained a `max_pairs=10000` caveat, produced only one non-singleton group under the current combined grouping output, and could not use source-aware grouping because the source field is too coarse. The feature surface is narrow. The model set is baseline-oriented. No wet-lab validation is included. The manuscript does not establish experimental transport confirmation, mechanism, broad delivery generalization, robust generalization, clinical interpretation, therapeutic relevance, or SOTA predictor status.

## Discussion

The present results are meaningful primarily because they are embedded in a benchmark-first and provenance-aware evidence surface. The repository distinguishes imported historical artifacts from regenerated current-contract artifacts and preserves caveats around dataset provenance, label-source criteria, leakage findings, and validation status. Within that framework, Logistic Regression and Random Forest exceeding the trivial class-prior baseline supports a cautious computational filtering interpretation on the present benchmark surface. The leakage-aware sensitivity rerun further supports the view that the baseline signal did not collapse under the committed group-stratified manifest. It does not establish delivery performance, biological actionability, leakage-free status, or robust generalization.

## Conclusion

The present work contributes an initial computational evidence package for a BBB-oriented peptide benchmark built around sequence-derived physicochemical features and reproducible baseline reruns. Its value lies in combining a bounded benchmark surface with explicit provenance boundaries, a clear imported-versus-regenerated distinction, and a leakage-aware sensitivity check showing that the baseline signal did not collapse under one committed group-stratified manifest. Future progress still depends on stronger provenance closure, richer grouping metadata, final metadata/reference review, and eventual experimental validation pathways. Until then, this package should be read as benchmark documentation and hypothesis-generation support, not as validated biological delivery evidence.
