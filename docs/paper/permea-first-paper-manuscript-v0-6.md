...
The study uses existing repository artifacts only. Models were not rerun for this draft, metrics were not recalculated for this draft, and no new split generation was performed. Metric values are copied from committed result artifacts and prior audit reports. Version v0.6 is a wording and source-to-claim clarity revision over v0.5, not a new experiment.
...
This manuscript draft is an internal first-paper v0.6 preparation draft from existing materials. It does not declare public bioRxiv readiness and does not imply external expert review or peer review.
